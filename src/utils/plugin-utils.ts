...
export function getDecoratorOrUndefinedByNames(
    ...
    return (decorators || ts.createNodeArray()).find((item) => names.includes(getDecoratorName(item)));
}

export function getTypeReferenceAsString(type: ts.Type, typeChecker: ts.TypeChecker): string {
    if (isArray(type)) {
    ...
    if (isPromiseOrObservable(getText(type, typeChecker))) {
        ...
        const elementType = getTypeReferenceAsString(head(typeArguments), typeChecker);
        if (!elementType) {
            return undefined;
        }
        return elementType;
 ...
 * @param typeChecker
 */
export function isAutoGeneratedEnumUnion(type: ts.Type, typeChecker: ts.TypeChecker): ts.Type {
